...
OdometryContainer::OdometryContainer (unsigned int s, double mta, double mra) 
throw (std::bad_alloc) : odo(s), drv(s), n(s), max_acc (mta), max_racc (mra) 
{
...
RobotLocation OdometryContainer::add_movement (RobotLocation start, Time t1, Time t2) const throw () 
{
  if (t1>=t2) 
  {
    ...
    if (odoindex+1<n) 
	{
      ...
      start.vtrans = odo[odoindex].vtrans.rotate (start.heading+start.vrot*
					t2.diff_msec(odo[odoindex].timestamp)*1e-3);
      return start;
    }
