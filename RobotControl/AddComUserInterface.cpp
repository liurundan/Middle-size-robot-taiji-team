...
AddComUserInterface::AddComUserInterface ( ConfigReader& conf, Player& _pl, ImageProcessing& _ip, CDC& dc) 
throw (RobotsException, std::bad_alloc) : pl(_pl), ip(_ip), commFailCount(0), commMaxFailCount(20)
{
	string confline;
	...
	{
		the_local_user_interface = new TerminalUserInterface(conf, pl, ip, dc );
	}
	else 
	{
	...
	the_communication = RobotsUdpServer::get_instance_pointer();
	the_communication->receive_all();
}


AddComUserInterface::~AddComUserInterface () throw () 
{
	delete the_local_user_interface;
	RobotsUdpServer::delete_instance();
}


bool AddComUserInterface::process_messages () throw () 
{
	...
	if (the_communication->receive_all())
	{
		...
		if(the_communication->getGameState(new_game_state))
			if ((new_game_state.refstate != MWM.get_game_state().refstate) ||
				(new_game_state.in_game != MWM.get_game_state().in_game)) 
			{
			...
			if(the_communication->getOwnHalf(new_oh))
				...
				(WorldModel::get_main_world_model()).get_message_board().receive (msgList);  
    }
	else 
	{
...
void AddComUserInterface::Setmessage(char c)
{
	the_local_user_interface->Setmessage(c);
}