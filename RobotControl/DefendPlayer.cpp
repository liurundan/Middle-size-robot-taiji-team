...
DefendPlayer::DefendPlayer( ConfigReader& cfg)
{
...
void DefendPlayer::getCmd(DriveVector *dv, const Time& t)
{
	decide_action(dv,t);
    //test_skills(dv,t);
}

void DefendPlayer::test_skills(DriveVector* dv,const Time& t)
{ 
 ;
}

void DefendPlayer::decide_action( DriveVector* dv,const Time& t )
{
	...
	if( in_play_flag == 1 )
	{
		...
		else // not fast to ball, go to best pos or defense
		{
			if( Tools::in_own_penalty_area(posBall) )
			{
				if( role == Defend1 || role == Defend3 ) 
			...
			else if( posBall.x < -5000 ) // get ball, not consider anything
			{
				if( role == Defend2 && posMe.getDistanceTo(posBall) < 2000 &&
					!WBOARD->teamPossesBall() )
				{
					if( fabs(angBall.get_deg_180()) < 110 && 
						fabs(posBall.y) < 1600 )
					{
    ...
    if( posBall.x > -6000 ) 
	{
        if( role == Defend3 || role == Defend1 )
		{
			if( fabs(MWM.ball2Robot.angle) < 0.2 &&	MWM.ball2Robot.distance < 550  )
			{
			//	WSpset p = MWM.valid_opponents;
			//	p.keep_players_in_circle(posBall,500);
			//	int num = p.num;
