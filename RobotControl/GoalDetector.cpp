...
void GoalDetector::searchGoals(const Image& image, Time time,
    ...
    throw (RobotsException)
{
    ...
    Vec tor1( worldModel.get_field_geometry().field_length/2., 0 );
    Vec tor2( worldModel.get_field_geometry().field_length/-2., 0 );

    Vec dirV1 = Vec((tor1 - location.pos).rotate(-location.heading)); // relative
    Vec dirV2 = Vec((tor2 - location.pos).rotate(-location.heading)); // direction

    Vec tor1img;
    Vec tor2img;

    if (mapping != 0) 
	{ // if inverse mapping is available use it!
	...
	{              // use an aproximation (magic value by S. Welker)
    ...
    if (vis) 
		vis = 0;       // finished visualization
}

void GoalDetector::visualizeGoal(const Vec& pos, int col)
{
    Painter p(*vis);
...
int GoalDetector::getDominantColor(const Image& image, const Vec& position, 
				 int n)
{
    int c = 0;
    int blue = 0;
    ...
    if ( yellow > n * 0.25 && blue < n * 0.1 )  // n = 80
	{
      return COLOR_YELLOW;
    }
