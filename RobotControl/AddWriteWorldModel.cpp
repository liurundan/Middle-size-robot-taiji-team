...
AddWriteWorldModel::AddWriteWorldModel ( ConfigReader& reader, 
										WorldModelType* wm) throw () : the_world_model (wm) 
{
  string fbasename;
  ...
  const FieldGeometry& fg = the_world_model->get_field_geometry(); // edit by lrd
  ...
  SYSTEMTIME tv;
  nulltime.get (tv);
  (*log_out) << "StartingTimeval " << tv.wSecond << ' ' << tv.wMilliseconds*1e3 << '\n';
}
...
void AddWriteWorldModel::set_compassdir (Angle h, Time t) throw ()
{
  Time now;
  the_world_model->set_compassdir (h,t);
}
