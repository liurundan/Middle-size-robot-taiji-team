...
ImageParticleTracker::ImageParticleTracker(
						 int colClass,
      ...
      windowHeight(windowHeight), n(n), minParticles(minParticles),  vis(0)
{
    ...
    max_x = 0.5*fg.field_length+fg.goal_band_width;
    max_y = 0.5*fg.field_width+fg.side_band_width;
}

void ImageParticleTracker::addParticle(const Vec& p)
{
    survivingparticles.push_back(p);
}

void ImageParticleTracker::propagate(const Image& image, 
...
void ImageParticleTracker::visualizeCenter()
{
    Painter p(*vis);
...
void ImageParticleTracker::visualizeSample(int x, int y, int c)
{
    Painter p(*vis);
...
bool ImageParticleTracker::found() const
{
    return survivingparticles.size() >= minParticles; 
}

Vec ImageParticleTracker::getPositionMean() const
{
    if (found()) 
	{
      return middle;
	...
	{
      throw RobotsException(__FILE__ ": No object found.");
    }
}

BallDetector::BallDetector(const CoordinateMapping* mapping, 
			     int centerX, int centerY, 
			     bool useRegion, bool useTrig,bool bfromUsb) 
    : mapping(mapping), tracker(COLOR_BALL, centerX, centerY), vis(0),
      centerX(centerX), centerY(centerY), useRegion(useRegion),
      useTrig(useTrig),bfromUsb(bfromUsb)
{
	if(bfromUsb)
	{
		tracker.setMinParticles(150);
	}
}
  
bool BallDetector::searchBall(const Image& image, 
    ...
    throw (RobotsException)
{
    ...
    bool useScanlinesOnlyIfBallNotSeen = false; // should not be changed.. 

    if (!tracker.found() || !useScanlinesOnlyIfBallNotSeen) 
	{ // if the particle tracker has not found the ball, add hypothesis from scanlines
    ...
    if (tracker.found()) 
	{
		...
		if (useRegion) 
		{
			...
			{  // have not found a pixel on the ball
			   /*
		...
		{  
			NearestPixelFinder npf(image, Vec(centerX, centerY),true);
			ballPosition = npf.find(ballPosition, COLOR_BALL);
			ballPositionWorld = mapping->map(ballPosition);
			if (useTrig) 
			{
...
bool BallDetector::searchUsbBall(const Image& image, 
			   const ScanResult* scanResult, Time time, 
			   VisibleObjectList* vol, bool writeWM)
    throw (RobotsException)
{
	...
	{
		Vec ballPosition = tracker.getPositionMean();
		Vec ballPositionWorld= mapping->map(ballPosition);//usb前向摄像头是靠建立映射表来查找距离的
 		
		if (vis) //Image* vis
		{
			visualizeBall(ballPosition);
		}
		if (vol) //VisibleObjectList* vol
		{
			vol->objectlist.push_back(
				VisibleObject (ballPositionWorld, VisibleObject::ball) );
		}
		if (writeWM) //bool writeWM
		{
			WorldModel::get_main_world_model().set_visual_information(
				VisibleObject (ballPositionWorld, VisibleObject::ball), time );
		}
...
InterceptTheoremCorrection::InterceptTheoremCorrection(double cameraHeight)
    : cameraHeight(cameraHeight)
{}
  
Vec InterceptTheoremCorrection::correct(const Vec& point, double height) const
{
...
Vec LookForColorInSurrounding::search(const Vec& start, int color, 
				    int maxDist) const
{
    int x = (int) start.x;
    int y = (int) start.y;
...
NearestPixelFinder::NearestPixelFinder(const Image& image,
					 const Vec& imageCenter,bool bfromUsb)
    : image(image), imageCenter(imageCenter),bfromUsb(bfromUsb)
{}

Vec NearestPixelFinder::find(const Vec& pos, int color)
{
