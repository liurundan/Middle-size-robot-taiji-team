...
public:
	...
	void setMinParticles( int mp ) { this->minParticles = mp;}
protected:
	int colClass;
    Vec center;
    int windowWidth;
...
{
public:
	NearestPixelFinder(const Image& image, const Vec& imageCenter,bool bfromUsb=false);
...
public:
	 BallDetector(const CoordinateMapping* mapping, 
		 ...
		 bool useRegion=true, bool useTrig=true,bool bfromUsb=false);
	 bool searchBall( const Image& image,
		               const ScanResult* scanResult, Time time = Time(),
		               VisibleObjectList* vol=0, bool writeWM = true)
      throw (RobotsException);
	 bool searchUsbBall( const Image& image,
		               const ScanResult* scanResult, Time time = Time(),
		               VisibleObjectList* vol=0, bool writeWM = true)
      throw (RobotsException);
