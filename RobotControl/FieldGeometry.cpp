...
FieldGeometry::FieldGeometry ( ConfigReader& vread) throw (InvalidConfigurationException) 
{
	...
	if( vread.get( "image_processing_type", stemp ) > 0 )
	{
		if (stemp == "DistanceCalibration")
		{
			if (vread.get ("DistanceCalibration::field_length", field_length)<=0 || field_length<0)
				throw  InvalidConfigurationException ("field_length");
			if (vread.get ("DistanceCalibration::field_width", field_width)<=0 || field_width<0)
				throw  InvalidConfigurationException ("field_width");
		}
	}
