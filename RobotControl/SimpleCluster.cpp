...
void VecCluster::update()
{
  ...
  if (minRadAng < 0.5 && maxRadAng > (0.9 *2 * M_PI))
  {
	  ...
	  {
		  double a = samples[j].angle().get_rad();
		  if (a < minRadAng && a > M_PI) 
			  minRadAng = a;
		  if (a > maxRadAng && a < M_PI) 
			  maxRadAng = a;
	  } 
      corrected = true;
  }
...
bool VecCluster::add(const  Vec& v)
{
  samples.push_back(v);
  update();
  return true;
...
double VecCluster::dist(const  Vec& v) const
{
  //double dist = fabs((v-com).length())-(comWidth/2.0);
  double dist = 10000;
  for (unsigned int i=0; i<samples.size(); i++)
  {
      if( ( (v-samples[i]).length() ) < dist )
		  dist = ((v-samples[i]).length());
...
double VecCluster::dist(const VecCluster& vc)
{
  ...
  for (unsigned int i=0; i<samples.size(); i++)
  {
      if( vc.dist(samples[i]) < dist ) 
		  dist = vc.dist(samples[i]);
  }
...
VecCluster VecCluster::glue(const VecCluster &other)
{
  VecCluster new_vc;
  for (unsigned int i=0; i< samples.size(); i++) 
	  new_vc.samples.push_back(samples[i]);
  for (unsigned int j=0; j< other.samples.size(); j++) 
	  new_vc.samples.push_back(other.samples[j]);
  new_vc.update();
  return new_vc;
}
...
void SimpleVecCluster::add(const  Vec& v)
{
  bool newClust = true;
