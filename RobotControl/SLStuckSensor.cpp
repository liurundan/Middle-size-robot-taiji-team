...
SLStuckSensor::SLStuckSensor (  ConfigReader& cfg) throw (std::bad_alloc) : n(15), oldpos(n), stuck_min (15)
{
...
void SLStuckSensor::update () throw () 
{
  ...
  if ( (fabs(dvrot)<2 && fabs(dvderiv)<2 && 3*c1<c2 && 0.5<c2 && fabs(c1-c2)>0.7) || 
	  (fabs(dvrot)<2 && fabs(dvderiv)<1 && 3*c1<c2 && 0.5<c2) ) 
  {
    count++;
    if (count>=stuck_min) 
	{
  ...
  was_stuck = (count>=stuck_min || timestamp_latest_stuck.elapsed_msec() <=700) ||
	  (was_stuck &&  timestamp_latest_stuck.elapsed_msec() <=5000 &&
	  (pos_latest_stuck-slpos).squared_length()<250000 &&
	  (slvel.angle ()-(pos_latest_stuck-slpos).angle()).in_between (Angle::seven_eighth, Angle::eighth));
}
...
RobotStuckLocation SLStuckSensor::get_stuck_location (Vec pos, Vec vtrans) const throw () 
{
  ...
  dest.robot_stuck |= (was_stuck && dest.msec_since_stuck<=5000 && 
	  (pos_latest_stuck-pos).squared_length()<250000 && 
	  (vtrans.angle ()-(pos_latest_stuck-pos).angle()).in_between (Angle::seven_eighth, Angle::eighth));
