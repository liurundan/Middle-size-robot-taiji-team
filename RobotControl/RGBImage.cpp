    ...
    : Image(), controlsBuffer(false)
{
    ...
    else 
	{
      const ImageConverter* convert = 
		  ImageConversionRegistry::getInstance()->getConverter(  buffer.format, ImageBuffer::FORMAT_RGB );
...
void RGBImage::getPixelYUV(int x, int y, YUVTuple* yuv) const
{
    int y0, u, v;
...
void RGBImage::getPixelUYVY(int x, int y, UYVYTuple* uyvy) const
{
...
void  RGBImage::getPixelRGB(int x, int y, RGBTuple* rgb) const 
{
    *rgb = reinterpret_cast<RGBTuple*>(buffer.buffer)[x+y*buffer.width];
}

unsigned char RGBImage::getPixelClass(int x, int y) const
{
    return classifier->lookup(
      reinterpret_cast<RGBTuple*>(buffer.buffer)[x+y*buffer.width]);
}

void RGBImage::setPixelYUV(int x, int y, const YUVTuple& yuv)
{
    RGBTuple& rgb = 
...
void RGBImage::setPixelRGB(int x, int y, const RGBTuple& rgb)
{
    reinterpret_cast<RGBTuple*>(buffer.buffer)[x+buffer.width*y] = rgb;
}

void RGBImage::setPixelUYVY(int x, int y, const UYVYTuple& uyvy)
{
