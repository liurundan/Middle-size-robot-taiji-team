...
void YUV2RGB::operator() (const ImageBuffer& src, ImageBuffer& dst) const
...
void  UYV2RGB::operator() (const ImageBuffer& src, ImageBuffer& dst) const
    throw (RobotsException)
{
...
void UYVY2YUV::operator() (const ImageBuffer& src, ImageBuffer& dst) const
    throw (RobotsException)
{
...
void  UYVY2RGB::operator() (const ImageBuffer& src, ImageBuffer& dst) const
    throw (RobotsException)
{
    if (src.format != ImageBuffer::FORMAT_YUV422 ||
	dst.format != ImageBuffer::FORMAT_RGB) 
...
ImageConversionRegistry* ImageConversionRegistry::singleton = 0;

ImageConversionRegistry* ImageConversionRegistry::getInstance()
{
    if (singleton == 0) 
	{
...
ImageConversionRegistry::~ImageConversionRegistry()
{
    for ( map<int,  map<int, ImageConverter*> >::iterator it = 
	   converterMap.begin(); 
	 it != converterMap.end(); 
	 it++) 
	 {
      
      for ( map<int, ImageConverter*>::iterator innerIt = 
	     it->second.begin(); 
...
void ImageConversionRegistry::registerConverter(ImageConverter* converter)
{
	converterMap[converter->getSourceFormat()]
	  [converter->getDestinationFormat()] = converter;
}

const ImageConverter* ImageConversionRegistry::getConverter(int srcFormat, int dstFormat) 
    throw(RobotsException)
{
