...
const unsigned char& HSISegmentationTool::lookup(const RGBTuple& rgb) const
{
	...
	{   
		for( unsigned int id = 1; id < colorClassInfoList->classList.size(); id++)
		{
    ...
    const_cast<HSISegmentationTool*>(this)->cTmp = c; // unpleasantly,once again ran
    return cTmp;
}
...
void HSISegmentationTool::load(string filename)
{
    ...
    if (!in)
	{
		throw new RobotsException("Could not read file.");
    }
...
void HSISegmentationTool::save(string filename) const
{
    ...
    if (! out ) 
	{
		throw new RobotsException("Could not write file.");       
    }
