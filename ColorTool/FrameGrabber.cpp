...
FrameGrabber::FrameGrabber() 
		: pBuffer(NULL),pBYTEbuffer(NULL),bufferSize(0), 
		flipImage(false), connected(false),bnotify(false),width(0), height(0)
{
...
void FrameGrabber::Init(int deviceId, bool displayProperties,
...
bool FrameGrabber::BindFilter(int deviceId, IBaseFilter **pFilter)
{
	if (deviceId < 0)
		return false;
...
void FrameGrabber::SetCrossBar(int fr, int iiw, int iih,CString mode)
{
...
HRESULT FrameGrabber::GrabByteFrame()
{
	HRESULT hr;
 	long    size = 0;
	long	evCode;
...
ImageBuffer FrameGrabber::getImage()
{
	HRESULT hr = S_OK;
 	hr = GrabByteFrame();
	if(FAILED(hr))
...
void FrameGrabber::FreeMediaType(AM_MEDIA_TYPE& mt)
{
    if (mt.cbFormat != 0) 
    {
...
void FrameGrabber::EnumDevices(DeviceInfo *head)
{
	if (!head)
		return;
...
void FrameGrabber::ErrMsg(LPTSTR szFormat,...)
{
...
void FrameGrabber::DisplayFilterProperties()
{
