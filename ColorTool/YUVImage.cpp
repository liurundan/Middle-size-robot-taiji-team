...
Image* YUVImage::clone() const
{
    ...
    : Image(), controlsBuffer(false)
  {
	...
	{
      const ImageConverter* convert = ImageConversionRegistry::getInstance()
								->getConverter(buffer.format, ImageBuffer::FORMAT_YUV444);
      // the preceeding call may have thrown an exception.

      this->buffer = ImageBuffer(buffer.width, buffer.height, ImageBuffer::FORMAT_YUV444,
  ...
  {
    this->buffer = ImageBuffer(width, height, ImageBuffer::FORMAT_YUV444,
  ...
  void YUVImage::getPixelYUV(int x, int y, YUVTuple* yuv) const
  {
    *yuv = reinterpret_cast<YUVTuple*>(buffer.buffer)[x+y*buffer.width];
  }					     
  
  void YUVImage::getPixelRGB(int x, int y, RGBTuple* rgb) const 
  {
    int y0, u, v, r, g, b;
  ...
  void YUVImage::getPixelUYVY(int x, int y, UYVYTuple* uyvy) const 
  {
  ...
  unsigned char YUVImage::getPixelClass(int x, int y) const
  {
    return classifier->lookup(
      reinterpret_cast<YUVTuple*>(buffer.buffer)[x+y*buffer.width]);
  }

  void YUVImage::setPixelUYVY(int x, int y, const UYVYTuple& uyvy)
  {
    int pos = y*buffer.width+x;
  ...
  void YUVImage::setPixelRGB(int x, int y, const RGBTuple& rgb)
  {
    int y0, u, v;
  ...
  void YUVImage::setPixelYUV(int x, int y, const YUVTuple& yuv)
  {
