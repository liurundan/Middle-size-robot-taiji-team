...
namespace 
{
  ...
  {
    return (fabs(vrot)>1.2 ? 0.8 : (fabs(vrot)>0.7 ? 0.9 : 1.0))*vrot;
  }

  inline Vector rectified_velocity (  Vector& vtrans) 
  {
    return (vtrans.getMagnitude()>1.2 ? 0.7 : vtrans*(vtrans.getMagnitude()>0.7 ? 0.8 : 1.0));
  }
}
...
OdometryContainer::OdometryContainer (unsigned int s, double mta, double mra) throw (std::bad_alloc) : odo(s), drv(s), n(s), max_acc (mta), max_racc (mra) {
  ...
  {
    odo[i].vtrans=drv[i].vtrans=Vector(0,0);
...
void OdometryContainer::add_odometry (DriveVector dv, MyTime t) throw ()
 {
  odo.get().timestamp=t;
...
void OdometryContainer::add_drive_vector (DriveVector dv, MyTime t) throw () 
{
  TDV latest(drv[-1]);
  double dt = t.diff_msec (latest.timestamp);
  double dvtrans = (latest.vtrans-dv.vtrans).getMagnitude();
  double dvrot = fabs(latest.vrot-dv.vrot);

  double mt = max_acc*dt*1e-3;
  if (mt<1e-3) mt=1e-3;
  if (dvtrans>mt)
    dv.vtrans = latest.vtrans*(1-mt/dvtrans)+dv.vtrans*(mt/dvtrans);
  mt = max_racc*dt*1e-3;
  if (mt<1e-3) mt=1e-3;
  if (dvrot>mt)
...
RobotLocation OdometryContainer::movement (MyTime t1, MyTime t2) const throw () {
  RobotLocation start;
  start.pos=Vector(0,0);
  start.heading=MyAngle::zero;
  start.vtrans=Vector(0,0);
  start.vrot=0;
  return add_movement (start, t1, t2);
}

RobotLocation OdometryContainer::add_movement (RobotLocation start, MyTime t1, MyTime t2) const throw ()
{
  if( t1 >= t2 ) 
  {
...
DriveVector OdometryContainer::get_odometry (MyTime t) const throw () 
{
...
DriveVector OdometryContainer::get_drive_vector (MyTime t) const throw () 
{
...
void OdometryContainer::Setodometry( RobotLocation rp )
{
	deltavalue.pos       = rp.pos;
	deltavalue.heading   = rp.heading;
}

RobotLocation OdometryContainer::Getodometry() const throw()
{
	return deltavalue;
}
