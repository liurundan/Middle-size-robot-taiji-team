...
void BallDetector::searchBall(const Image& image, 
    ...
    throw (RobotsException)
{
	...
	RobotLocation rloc;
	rloc.pos = Vec::zero_vector;
	rloc.heading = Angle::deg_angle(0);
    ...
    if (tracker.found()) 
	{
      ...
      if (writeWM) 
	  {
		//WorldModel::get_main_world_model().set_visual_information(
		  ...
		  pMainDlg->imgballpos = ballPosition;
		  pMainDlg->realballpos = ballPositionWorld;
      }
    }
    if (vis) 
	{
