...
/*
		...
		{
			LineSegment baseLine0(Vec(distance,-distance*0.5774),  Vec(distance, distance*0.5774) );
			stepAngle = Angle::deg_angle(360. / 6); 
			for(int j=0;j<6;j++)	//六边形
			{			
				insertScanLine (baseLine0.rotate (j*stepAngle).translate (middle),width,height);
			}
			distance+= 8;
		} 
*/
